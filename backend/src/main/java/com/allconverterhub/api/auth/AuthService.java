...
public class AuthService {
  ...
  public AuthResponse refresh(String refreshToken) {
    ...
    String tokenHash = hash(claims.getId());
    AuthToken token = tokens.findByTokenHashAndPurpose(tokenHash, TokenPurpose.REFRESH)
  ...
  public void verifyEmail(String rawToken) {
    AuthToken token = validOneTimeToken(rawToken, TokenPurpose.EMAIL_VERIFICATION);
    User user = users.findById(token.getUserId()).orElseThrow(() -> new IllegalArgumentException("User not found."));
    user.verifyEmail();
    users.save(user);
    tokens.delete(token);
  }
  ...
  public void requestPasswordReset(String email) {
    users.findByEmailIgnoreCase(normalize(email))
        .ifPresent(user -> sendOneTimeToken(user, TokenPurpose.PASSWORD_RESET));
  }

  @Transactional
  public void resetPassword(String rawToken, String password) {
    AuthToken token = validOneTimeToken(rawToken, TokenPurpose.PASSWORD_RESET);
    User user = users.findById(token.getUserId()).orElseThrow(() -> new IllegalArgumentException("User not found."));
    user.setPasswordHash(passwordEncoder.encode(password));
    users.save(user);
    tokens.deleteByUserIdAndPurpose(user.getId(), TokenPurpose.REFRESH);
    tokens.delete(token);
  }

  private AuthResponse createSession(User user) {
    JwtService.UserPrincipal principal = new JwtService.UserPrincipal(user.getId(), user.getEmail());
    String refreshId = UUID.randomUUID().toString();
    String refreshToken = jwtService.createRefreshToken(principal, refreshId);
    tokens.save(new AuthToken(user.getId(), hash(refreshId), TokenPurpose.REFRESH,
        Instant.now().plusSeconds(jwtService.getRefreshTokenSeconds())));
    return new AuthResponse(jwtService.createAccessToken(principal), refreshToken, "Bearer",
        jwtService.getAccessTokenSeconds());
  }

  private void sendOneTimeToken(User user, TokenPurpose purpose) {
    tokens.deleteByUserIdAndPurpose(user.getId(), purpose);
    String rawToken = UUID.randomUUID() + "-" + UUID.randomUUID();
    tokens.save(new AuthToken(user.getId(), hash(rawToken), purpose, Instant.now().plusSeconds(24 * 3600)));
  ...
  private AuthToken validOneTimeToken(String rawToken, TokenPurpose purpose) {
