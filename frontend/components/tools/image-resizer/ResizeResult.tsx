...
import type { ResizeResult as ResizeResultData } from "@/hooks/useImageResizer";

interface ResizeResultProps {
  result: ResizeResultData;
  onReplace: () => void;
}
...
export default function ResizeResult({ result, onReplace }: ResizeResultProps) {
  const download = () => {
  ...
  return (
    <section className="rounded-2xl border border-emerald-500/30 bg-slate-900 p-6">
      <h2 className="text-xl font-bold">Your resized image is ready</h2>
      <div className="mt-5 grid gap-6 md:grid-cols-[1fr_auto]">
        <div className="flex items-center gap-4">
          ...
          <img src={result.previewUrl} alt="Resized image preview" className="h-24 w-24 rounded-xl border border-slate-700 object-contain" />
          <div>
            <p className="font-semibold">{result.dimensions.width} × {result.dimensions.height}px</p>
            <p className="mt-1 text-sm text-slate-400">{formatSize(result.file.size)} · {result.file.type.replace("image/", "").toUpperCase()}</p>
          </div>
        </div>
